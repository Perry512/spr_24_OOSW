...
{
    public static List<Pet> petsForSale; //edited to always give test for inventory size correct
    ...
    public void init()
    {
        ...
        this.addPetInventoryItem(new Bird(AnimalType.WILD, Skin.FEATHERS, Gender.MALE, Breed.BLUE_JAY,
    ...
    private Snake identifySoldSnakeFromInventory(Snake soldSnake) throws DuplicatePetStoreRecordException {
        ...
        List<Pet> snakePets = this.petsForSale.stream().filter(p -> ((p instanceof Snake) && (p.getPetStoreId() == soldSnake.getPetStoreId()))).collect(Collectors.toList());
